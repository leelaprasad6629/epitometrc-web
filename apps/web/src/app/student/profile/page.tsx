...
import { 
  ...
  Target, FileText, Link2, Compass, GraduationCap, Trophy, Users, HeartHandshake, HelpCircle
} from "lucide-react";
    ...
    <div className="relative min-h-screen bg-slate-50/30 overflow-hidden py-8 px-4 sm:px-6 lg:px-8 text-left text-slate-700 font-sans text-xs">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
            ...
            {isEditing ? (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 font-bold transition-all shadow-xs"
                >
        ...
        {!verified ? (
          ...
          /* Dynamic AI workspace showing only non-empty parsed sections */
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="pt-4 border-t border-slate-50">
                      {isEditing ? (
                        <textarea
                          ...
                          className="w-full rounded-2xl border border-slate-200 p-3.5 text-xs focus:border-slate-800 outline-none h-24 resize-none bg-white font-medium"
                        />
                      ) : (
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-3">
                            {educationList.map((item, idx) => (
                              <div key={idx} className="p-3 bg-slate-50/50 border border-slate-100 rounded-2xl relative">
                                {isEditing ? (
              ...
              {/* Dynamic Job Matching & Analysis widget */}
            ...
            <div className="space-y-6">
              ...
              {/* Profile Health Completeness Gauge */}
              <DashboardCard glowColor="indigo" className="text-left flex items-center justify-between gap-4">
                <div className="space-y-1.5 flex-1 font-sans">
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 pt-1 text-[9px] text-slate-500 font-semibold">
              ...
              {hasCertsAwards && (
                <DashboardCard glowColor="purple" className="space-y-4">
                  <h3 className="font-display text-[10.5px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                    <Award className="h-4 w-4 text-cyan-600" /> Certs & Achievements
                  </h3>
                  <div className="pt-4 border-t border-slate-50 space-y-4">
                    {/* Certifications list */}
                  ...
                  {!collapsedSections.academic && (
                    <div className="pt-4 border-t border-slate-50 space-y-4">
                      ...
                      {leadershipRolesList.length > 0 && (
                        <div className="space-y-1.5">
                          <span className="font-bold text-slate-400 text-[9px] uppercase">Leadership</span>
                          {leadershipRolesList.map((item, idx) => (
                            <div key={idx} className="p-2.5 bg-slate-50/50 border border-slate-100 rounded-xl">
                              <h5 className="font-bold text-slate-700">{item.role}</h5>
                              <p className="text-slate-450 mt-0.5">{item.organization} • {item.duration}</p>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      {volunteerExperienceList.length > 0 && (
                        <div className="space-y-1.5">
                          <span className="font-bold text-slate-400 text-[9px] uppercase">Volunteering</span>
                          {volunteerExperienceList.map((item, idx) => (
                            <div key={idx} className="p-2.5 bg-slate-50/50 border border-slate-100 rounded-xl">
                              <h5 className="font-bold text-slate-700">{item.role}</h5>
                              <p className="text-slate-450 mt-0.5">{item.organization}</p>
                            </div>
                          ))}
                        </div>
                      )}
