...
import { Briefcase, Calendar, CheckCircle2, ChevronRight, Clock, MapPin, Loader2, Inbox } from "lucide-react";
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-500 text-sm font-medium animate-pulse">Retrieving your job applications...</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="space-y-6">
        {applications.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-12 text-center text-slate-400 space-y-3">
            <Inbox className="h-10 w-10 text-slate-350 mx-auto" />
            <h3 className="font-display font-bold text-slate-700 text-sm">No Applications Filed Yet</h3>
            <p className="text-xs text-slate-450 max-w-sm mx-auto">
              Browse our Careers Portal and apply for open engineering or strategy roles to track your process pipeline here.
            </p>
            <Button href="/jobs" variant="primary" className="h-9 px-4 rounded-xl text-xs font-bold mt-2">
              Browse Open Jobs
            </Button>
          </div>
        ) : (
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  {app.steps.map((step: any, idx: number) => (
                    <div key={idx} className="relative flex gap-3 sm:flex-col sm:gap-2 text-left">
