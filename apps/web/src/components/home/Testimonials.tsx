        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {loading ? (
            Array.from({ length: 3 }).map((_, idx) => (
              <div key={idx} className="bg-slate-900/50 border border-slate-800 rounded-2xl p-8 space-y-6 animate-pulse">
                <div className="flex gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div key={i} className="h-4 w-4 bg-slate-800 rounded-full" />
                  ))}
                </div>
                ...
                <div className="h-8 bg-slate-800 rounded w-1/3 pt-4 border-t border-slate-800" />
              </div>
            ))
          ) : (
            testimonials.map((item, idx) => (
              <motion.div
                ...
                className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 flex flex-col justify-between hover:border-slate-700 transition-all hover:bg-slate-900/80 hover:-translate-y-1 shadow-xl shadow-black/25"
              >
                <div>
                  ...
                  <p className="text-slate-300 text-sm font-sans leading-relaxed italic mb-8">
                ...
                <div className="border-t border-slate-800/80 pt-5 flex items-center justify-between">
                  ...
                  <MessageSquare className="h-4 w-4 text-orange-500/30" />
                </div>
              </motion.div>
            ))
          )}
